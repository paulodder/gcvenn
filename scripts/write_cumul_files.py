...
def run(*args):
    ...
    with open('genes/static/cumulative_max.txt', 'w') as f:
        f.write('interval\tnofgenes_jan\tnofgenes_gte\tnofgenes_tcgan\n')
        cur = 0
        ...
        while cur < 13:
            f.write('{:.2f}'.format(cur)+
                    '\t'+str(len([g for g in jans if g<= cur]))+
                    '\t'+str(len([g for g in gtes if g<= cur]))+
                    '\t'+str(len([g for g in tcgans if g<= cur]))+
                    '\n')
            cur = round(cur +0.01, 2)
            # Write final line manually because graph stops at 13
        f.write('{:.2f}'.format(cur)+'\t15064\t15064\t15064')
